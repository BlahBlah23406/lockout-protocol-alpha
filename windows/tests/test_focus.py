"""Tests for focus mode: sessions, the provider layer, and the judgement log.

These cover the parts where a bug is expensive and silent — the wire format for each provider,
the rule that a backend failure never becomes a block, and the session watchlist arithmetic that
decides whether an app is looked at at all. None of them touch the network or the screen.
"""
...
class TestFocusSession(unittest.TestCase):
    ...
    def test_watchlist_tracks_later_edits_to_the_defaults(self):
        """The session stores a delta, not a snapshot: editing your default list mid-session is
    ...
    def test_unknown_accountability_falls_back_to_self(self):
        """Fail towards the *weaker* mode. A corrupt prefs file must not silently promote a user
...
class TestRequestBuilding(unittest.TestCase):
    ...
    def test_a_base_url_already_ending_in_v1_is_not_doubled(self):
        """The single most common misconfiguration: pasting LM Studio's http://…:1234/v1 and
        getting /v1/v1/chat/completions."""
...
class TestNeverBlockOnFailure(unittest.TestCase):
    """The load-bearing safety property: nothing that goes wrong on our side may look like the
    user being off task."""
    ...
    def test_every_key_exhausted_is_transient_not_a_block(self):
        """Running out of API credit is our problem. It must cost the user nothing."""
        rejected = providers._Attempt(
...
class TestLearnedPolicyClient(unittest.TestCase):
    """The learner is experimental and generated by a separate program, so the client must treat
    ...
    def test_threshold_can_never_exceed_the_client_ceiling(self):
        """Hand-editing the generated file must not be able to switch blocking off."""
